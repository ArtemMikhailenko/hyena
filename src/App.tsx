...
import { rabbitPhoto } from "./images"; // Added 'tonIcon'
...
const TaskItem: React.FC<TaskItemProps> = ({
  ...
  return (
    <div
      ...
      className={`flex items-center justify-between border border-white/40 bg-opacity-60 backdrop-filter backdrop-blur-lg rounded-xl p-2 mb-3 shadow-lg transition-shadow duration-300 ${
    ...
    >
      {icon ? (
        <div className="flex items-center gap-2 z-50">
          <div>
            {typeof icon === "string" ? (
              <img
                className="h-11 w-11 rounded-full border-2 border-rabbitAccent"
                src={icon}
                alt={title}
              />
            ) : (
              <div className="text-[22px] px-[8px] rounded-lg py-2 text-white bg-rabbitAccent">
                {icon}
              </div>
            )}
          </div>
      ...
      <div className="text-gray-300 z-50">
        {status === "completed" && (
          <img src={doneIcon} alt="Done" className="w-6 h-6 z-50" />
        )}
        {status === "loading" && <div className="loader"></div>}
        {status === "claimable" && onClaim && (
          <button
            onClick={onClaim}
            className="bg-rabbitAccent text-white px-4 py-1 rounded-full hover:bg-orange-600 transition-colors duration-300"
          >
            Claim
          </button>
        )}
        {status === "not_started" && reward !== undefined && (
          <>
            <button
              className={`flex items-center justify-center text-black sm:px-4 rounded-full`}
            >
              {title === "Join Telegram channel" ||
              title === "Join our Telegram Group" ? (
                <div className="text-xs bg-black text-white w-[50px] py-1 rounded-full">
                  Join
                </div>
              ) : title === "Follow on X" ? (
                <div className="bg-black text-white w-[50px] py-1 rounded-full flex items-center justify-center">
                  <BsTwitterX size={14} />
                </div>
              ) : title === "Make TON Great Again" ? (
                <img src={tonIcon} alt="TON" className="w-8 h-8" />
              ) : (
                <BsLightningChargeFill className="w-8 h-8 text-white" />
              )}
            </button>
          </>
        )}
...
const App: React.FC = () => {
  ...
  const transaction = {
    ...
    messages: [
      {
        address: "UQB40jZEYCTePhJPzTEISyirThOtTfjLF9A_XtUMUh61q6vX", // Receiver's wallet address
  ...
  const handleTransactionClick = async (reward: number) => {
    ...
    try {
      ...
      if (res) {
        ...
        showAlert("Transaction was successful! You've earned 10,000 HYENA.");
    ...
    const initializeTelegram = async () => {
      try {
        ...
        const user = {
          ...
          userid: initDataUnsafe.user.id ? initDataUnsafe.user.id.toString() : "",
        ...
        // Если кошелек не подключен, сразу запрашиваем подключение
        if (!address) {
  ...
  const fetchOrAddUser = async (
    ...
    try {
      ...
      const response = await fetch(
        `https://backend.hyenatongame.com/get_user?UserId=${userid}`,
        {
          headers: {
            "X-Telegram-Init-Data": initData
          }
      ...
      if (response.ok) {
        ...
        await addUser(userid, startparam, username , address);
      } else {
  ...
  const addUser = async (
    ...
    try {
      const response = await fetch(
        "https://backend.hyenatongame.com/add_user",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const saveTaskCompletion = async (
    ...
    try {
      const response = await fetch(
        "https://backend.hyenatongame.com/update_user",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleTelegramTaskClick = async (taskKey: string, link: string) => {
    ...
    setTimeout(async () => {
      ...
      try {
        const response = await fetch(
          `https://backend.hyenatongame.com/check_telegram_status?user_id=${userId}&chat_id=${chatId}`,
          {
            headers: {
              "X-Telegram-Init-Data": initData
            }
  ...
  const handleFetchedTaskClaim = async (task: any) => {
    try {
      ...
      const markDoneResponse = await fetch(
        "https://backend.hyenatongame.com/mark_task_done",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const increasePointsResponse = await fetch(
        "https://backend.hyenatongame.com/increase_totalgot",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const checkDailyRewardStatus = async () => {
    try {
      const initData = window.Telegram.WebApp.initData || "";
      const response = await fetch(
        "https://backend.hyenatongame.com/gamer",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        const response = await fetch(
          `https://backend.hyenatongame.com/get_user_tasks?userid=${userID}`,
          {
            headers: {
              "X-Telegram-Init-Data": initData
            }
        ...
        <>
          <div className="mt-6">
            ...
            <TaskItem
              ...
              onClick={() =>
                handleTaskClick("task7", "https://twitter.com/Hyenatapgame")
              }
            ...
            <TaskItem
              ...
              onClick={() =>
                handleTaskClick("task14", "https://www.YouTube.com/@Laughinghy")
              }
  ...
  return (
    <div className="relative flex justify-center min-h-screen ">
      {loading ? (
        ...
        <>
          {/* Main Content */}
          <div className="relative w-full text-white font-bold flex flex-col max-w-xl z-50">
            {activePage === "home" && (
              <>
                <div
                  className="min-h-screen pb-20 pt-4 md:pt-8 px-2 relative bg-fixed bg-cover bg-center bg-no-repeat"
                  style={{
                ...
                >
                  ...
                  {/* Top Box */}
                  <div className="relative flex flex-col items-center z-50 mb-5">
                    {/* <div className="absolute right-10 bottom-[4.5rem] animate-pulse">
                      <img
                        src={rabbitPhoto}
                        alt="rabbit"
                        className="w-16 md:w-20 h-20 md:h-20 rounded-3xl opacity-100"
                      />
                    </div>
                    <div className="absolute left-10 bottom-[4.5rem] animate-pulse">
                      <img
                        src={rabbitPhoto}
                        alt="rabbit"
                        className="w-16 md:w-20 h-20 md:h-20 rounded-3xl opacity-100"
                    ...
                    <img
                      src={rabbitPhoto}
                      alt="Token Logo"
                      className="w-52 h-52 rounded-3xl animate-pulse  p-4 shadow-md"
                    />

                    <p className="text-3xl mt-2 text-white font-bold">
                      {points.toLocaleString()} HYENA
                    </p>
                  </div>

                  {/* Segment Switcher */}
                  <div className="flex justify-center mb-4 mt-2 z-50">
                    <div className="flex bg-rabbitPrimary rounded-xl overflow-hidden shadow-lg w-full max-w-md">
                      <button
                        className={`flex-1 z-50 py-2 px-4 text-center whitespace-nowrap text-sm sm:text-base transition-all duration-300 ${
                          taskSegment === "LIONS"
                            ? "bg-rabbitAccent text-white"
                            : "text-white hover:bg-rabbitAccent"
                        }`}
          ...
          {/* Navbar */}
          {activePage !== "game" && (
            <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 w-full md:max-w-xl rounded-t-2xl bg-[#2E2214] flex justify-around items-center z-50 text-xs">
              <div
